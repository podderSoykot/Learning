    ...
    <section id="blog" className="py-20">
      <div className="container">
        <div className="text-center mb-16">
          <p className="text-sm font-medium text-accent mb-4">Industry Insights</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {blogPosts.map((post, index) => (
            <Card key={index} className="border-0 shadow-lg hover:shadow-xl transition-shadow cursor-pointer group">
              ...
              <CardHeader>
                <div className="flex items-center space-x-4 text-sm text-muted-foreground font-serif mb-2">
                ...
                <CardTitle className="font-sans group-hover:text-accent transition-colors">{post.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <CardDescription className="font-serif">{post.description}</CardDescription>
              </CardContent>
            </Card>
          ))}
        </div>
